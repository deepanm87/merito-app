...
export default function ApproveRejectButtons({
  ...
  const handleStatusUpdate = async ({
    ...
    const result = await updateTestimonialStatusAction({
      testimonialId,
      status,
      formId,
    });
  ...
  return (
    <div className="flex gap-2">
      <Button
        className="bg-gray-50 border border-green-200 text-green-500 hover:bg-green-100"
        onClick={() =>
          handleStatusUpdate({ status: TestimonialStatus.APPROVED })
        }
      >
      ...
      <Button
        ...
        onClick={() =>
          handleStatusUpdate({ status: TestimonialStatus.REJECTED })
        }
      >
